...
<head>
		...
		function update(){
			if($("#name").val()==""){
				$.message("warn","专业名称不能为空");
				return false;
			}
			$.ajax({
				url:"/bsxykq/admin/major/major_update.jhtml",
	...
	<body class="no-skin" style="background-color:white;overflow-x:hidden">
		...
		<table class="input" style="margin-left:47px;">
			<tr>
				<th style="width:80px">
					<span class="requiredField">*</span>专业名称:
				</th>
				<td  width="270">
					<input class="text" name="name" id="name" value="${map.name}"/>
				</td>
				<th style="width:80px">
					<span class="requiredField">*</span>所属学院:
				</th>
				<td  width="270">
					<select name="institute" class="text">
					[#list list as data]
						<option value="${data.id}" [#if data.id == map.institute] selected="selected" [/#if]>${data.institute}</option>
						[/#list]
					</select>
				</td>
			</tr>
